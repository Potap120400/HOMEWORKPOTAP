...
function getSubjects(student) {
  const subName = Object.keys(student.subjects);
  const result = subName.map((sub) => (sub.charAt(0).toUpperCase() + sub.slice(1)).replaceAll('_', ' '));
  return result;
};
...
const getAverageMark = (students ) => {
  ...
  const sum = evaluation.reduce((first, second) => first+second)
  return (sum/evaluation.length).toFixed(2);
}     
...
// 4
const getStudentsNames = (students) => students.map((alphabet) => alphabet.name).sort();
